 ...
 */
/**
 * 조사·서술격 어미 목록. 긴 것부터 검사한다.
 *
 * 이 목록이 없으면 BM25가 무너진다. 한국어는 교착어라 같은 명사가
 * 상품이/상품은/상품을/상품에/상품으로처럼 수십 가지 표기형으로 갈라지고,
 * 각 표기형은 개별적으로 희귀해진다. IDF는 희귀함을 곧 정보량으로 읽으므로
 * 조사가 만든 가짜 희소성이 주제어를 압도한다.
 *
 * 실측 — "제 상품이 위험한 편인가요?" (수정 전):
 *   상품이  df=  4  idf=4.43  기여 6.17   ← 표기형
 *   품이    df= 20  idf=2.91  기여 4.06   ← 조사에 걸친 bigram
 *   상품    df=352  idf=0.07  기여 0.15   ← 진짜 어간
 *   위험    df=224  idf=0.52  기여 0.99   ← 진짜 주제어
 * 11.4점 중 10.2점이 '상품이'라는 표기에서 나와, 위험 고지 문단(위험 10회)이
 * 변경절차 문단(위험 1회)에 밀렸다.
 */
const PARTICLES = [
  // 3음절
  "으로는", "으로도", "으로서", "으로써", "에서는", "에서도", "에게는",
  "이라는", "이라고", "에게서",
  // 2음절
  "에서", "에게", "으로", "이나", "이란", "이든", "이며", "라는", "라고",
  "보다", "부터", "까지", "마다", "조차", "밖에", "만큼", "처럼", "한테",
  "께서", "라도",
  // 1음절
  "이", "가", "은", "는", "을", "를", "에", "의", "도", "로", "와", "과",
  "만", "랑",
  // 관형형·연결형 어미 — 조사와 같은 규칙(어간이 짧으면 원형 유지)으로 다룬다.
  // "위험한"→"위험", "발생할"→"발생", "보호되지"→"보호".
  // 2글자 명사는 보호된다: "제한"·"분할"·"참고"는 어간이 한 글자라 그대로 남는다.
  //
  // "할"을 빼먹었을 때 "원금 손실이 발생할 수 있나요"의 근거가 과세 문단으로
  // 잡혔다. '발생할'·'생할'이 살아남아 한 단어가 토큰 세 개로 계산되는 바람에,
  // 세금 문장이 '원금·손실·발생'을 모두 담은 문장과 3:3 동점이 됐다.
  "하는", "되는", "하며", "되며", "하여", "되어", "하지", "되지",
  "하고", "되고", "해야", "할", "될", "한", "된",
];

/**
 * 의문·종결 어미. 조사와 달리, 떼고 나서 어간이 짧으면 **토큰을 버린다.**
 *
 * 조사는 명사에 붙으므로 남는 어간이 짧으면 원형이 진짜 명사일 가능성이
 * 높다(물가·성과·제한). 반면 종결 어미가 붙은 어절은 서술어이고, 어간이
 * 한 글자면 남길 내용이 없다.
 *
 * 이것을 빼먹으면 조사 정규화가 오히려 해롭다. 실측 — 조사만 뗐을 때
 * "제 상품이 위험한 편인가요?"의 1위가 펀드 결산·환매 문단이었다.
 * '편인가요'가 남긴 bigram '인가'·'가요'가 문서의 "가능한가요"·"무엇인가요"와
 * 맞으면서, 정작 주제어 '위험'(idf 0.52)을 눌렀기 때문이다.
 * '편인가요'는 어간이 '편' 한 글자이므로 통째로 버려야 한다.
 */
const ENDINGS = [
  "입니까", "습니까", "인가요", "은가요", "는가요", "한가요", "을까요",
  "되나요", "하나요", "시나요", "나요", "가요", "까요", "네요", "군요",
  "해요", "예요", "에요",
  // 서술 종결형. 문서 쪽 어절이 대부분 여기 해당한다.
  "합니다", "됩니다", "입니다", "습니다", "십시오",
];

/**
 * 어절에서 조사를 떼어 어간만 남긴다. 색인과 질의에 똑같이 적용해야
 * 의미가 있다 — 한쪽만 정규화하면 매칭이 오히려 줄어든다.
 *
 * 남는 어간이 2글자 미만이면 떼지 않는다. 이 한 줄이 과도한 절단을 막는다:
 * 물가·성과·결과·평가·제도·한도·주의는 모두 조사 음절로 끝나지만
 * 어간이 한 글자가 되므로 원형 그대로 남는다.
 *
 * 두 번까지 반복한다. '위험도가'를 '위험도'에서 멈추면 문서의 '위험도'와는
 * 맞아도 '위험'과 어긋나, 같은 단어가 표기에 따라 갈리는 문제가 남는다.
 */
function stripParticles(word: string): string {
  // 종결 어미가 먼저다. "위험한가요"는 어미 "한가요"로 잘라야 "위험"이 되고,
  // 조사 "가"부터 보면 "위험한"에서 멈춘다.
  const ending = ENDINGS.find((e) => word.endsWith(e));
  if (ending) {
  ...
  for (let round = 0; round < 2; round++) {
    const hit = PARTICLES.find(
      (p) => w.length - p.length >= 2 && w.endsWith(p),
    );
    if (!hit) break;
...
export function tokenize(text: string): string[] {
  ...
  for (const run of runs) {
    if (/^[가-힣]/.test(run)) {
      ...
      // 조사를 떼고 어간으로 색인한다. bigram도 어간에서 만들어야
      // '품이'처럼 명사와 조사에 걸친 조각이 생기지 않는다.
      const stem = stripParticles(run);
      // 어간이 비었다 = 내용 없는 서술어("편인가요", "되나요")
